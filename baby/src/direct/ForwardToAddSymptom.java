...
/**
 * Servlet implementation class ForwardToAddSymptom
 */
public class ForwardToAddSymptom extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		String str2=request.getSession().getAttribute("str2").toString();
		String chufang=request.getSession().getAttribute("chufang").toString();
		String shoufa=request.getSession().getAttribute("shoufa").toString();
		String selplan = request.getParameter("selplan");
		...
		//限制添加病例开始
		String xianzhiTime = ddfTime+"%";
		System.out.println("时间为："+xianzhiTime);
		int intpid=Integer.parseInt(pid);
		RecordDao recsan=new RecordDaoImpl();
		int xianzhi=recsan.getRecord(xianzhiTime,intpid);
		System.out.println("返回的孩子病例数"+xianzhi);
		
		if(xianzhi>=1){
			out.println("<meta   http-equiv='Content-Type'   content='text/html;   charset=UTF-8'>");
			out.println("<script>");
			out.println("alert('对不起，此宝宝在一分钟内不能连续添加病例！');");
		...
		String zzxw=request.getSession().getAttribute("zzxw").toString();
		String miansexw=request.getSession().getAttribute("miansexw").toString();
		String xydxw=request.getSession().getAttribute("xydxw").toString().trim();
//		String chunsexw=request.getSession().getAttribute("chunsexw").toString();
//		String shetixw=request.getSession().getAttribute("shetixw").toString();
//		String shezhixw=request.getSession().getAttribute("shezhixw").toString();
//		String shetaixw=request.getSession().getAttribute("shetaixw").toString();
		
		request.getSession().setAttribute("pname", pname);
		request.getSession().setAttribute("pborn", nowTime);
		...
		rd.setCtime(nowTime);
		rd.setPid(pidint);
		rd.setUserid(useridint);
		rd.setFid(fidint);
		rd.setDyzz(str2);
		rd.setDycf(chufang);
		rd.setDysf(shoufa);
//		rd.setDyzy(shxdzhuyi);
		rd.setDyzy(zhuyibh);
