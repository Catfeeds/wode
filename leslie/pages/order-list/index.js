...
Page({
  ...
  cancelOrderWaterTap: function (e) {
    var that = this;
    var orderId = e.currentTarget.dataset.id;
    wx.showModal({
      title: '确定要取消叫水吗？',
      ...
      success: function (res) {
        if (res.confirm) {
          wx.showLoading();
          wx.request({
            url: app.globalData.subDomain + '/order_water_cancel',
            data: {
              id: orderId
            },
            success: (res) => {
    ...
    wx.showModal({
      ...
      success: function (res) {
        if (res.confirm) {
          //wx.showLoading();
          wx.request({
            url: app.globalData.subDomain + '/order_water_confirm',
            data: {
              id: orderId,
              user_id: user_id
            },
            success: (res) => {
              if (res.data.code == 0) {
                that.onShow();
                wx.redirectTo({
                  url: '/pages/order-list/index?order_status=3',
                })
              }
            }
          })
  ...
  onShow: function() {
    ...
    postData.user_id = wx.getStorageSync('user_id');
    wx.request({
